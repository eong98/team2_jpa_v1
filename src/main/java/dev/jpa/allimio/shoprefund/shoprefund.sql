...
CREATE TABLE SHOP_REFUND (
  ...
  PNO            NUMBER(10)                      NULL, -- 연결된 환불 결제기록 (FK -> SHOP_PAYMENT.NO)
  ...
  CONSTRAINT FK_SHOP_REFUND_ONO FOREIGN KEY (ONO) REFERENCES SHOP_ORDER (NO),
  CONSTRAINT FK_SHOP_REFUND_PAYMENTNO FOREIGN KEY (PNO) REFERENCES SHOP_PAYMENT (NO),
...
COMMENT ON COLUMN SHOP_REFUND.ONO             IS '구독 내역 번호 (FK -> SHOP_ORDER.NO)';
