...
CREATE TABLE SHOP_PAYMENT (
    ...
    ONO         VARCHAR2(30)                NOT NULL, -- 구독내역 번호 (FK -> SHOP_ORDER.ORDERNO)
    ...
    CONSTRAINT FK_SHOP_PAYMENT_ONO FOREIGN KEY (ONO) REFERENCES SHOP_ORDER (ORDERNO),
...
COMMENT ON COLUMN SHOP_PAYMENT.ONO       IS '구독 내역 번호 (FK -> SHOP_ORDER.ORDERNO)';
...
CREATE INDEX IX_SHOP_PAYMENT_ONO ON SHOP_PAYMENT (ONO);
CREATE INDEX IX_SHOP_PAYMENT_MNO ON SHOP_PAYMENT (MNO);