CREATE TABLE SHOP_ORDER_LOG (
  ...
  ONO        VARCHAR2(30)                NOT NULL, -- 구독 내역 번호 (FK -> SHOP_ORDER.ono)
  ...
  CCNT NUMBER(2) NULL, -- 변경 전, 신규구매 지정된 CCTV 대수
  BPRICE  NUMBER(12,2) NULL,  -- 변경 후 CCTV 대수

  CONSTRAINT PK_SHOP_ORDER_LOG PRIMARY KEY (NO),
  CONSTRAINT FK_SHOP_ORDER_LOG_ONO FOREIGN KEY (ONO) REFERENCES SHOP_ORDER (NO)
);
...
COMMENT ON COLUMN SHOP_ORDER_LOG.CCNT IS '변경 전, 신규구매 지정된 CCTV 대수';
COMMENT ON COLUMN SHOP_ORDER_LOG.BPRICE  IS '변경 전, 신규구매 기본단가';
...
CREATE INDEX IX_SHOP_ORDER_LOG_ONO ON SHOP_ORDER_LOG (ONO);
CREATE INDEX IX_SHOP_ORDER_LOG_MNO ON SHOP_ORDER_LOG (MNO);



---------------------------------------------------------
ALTER TABLE SHOP_ORDER_LOG ADD (
  PNO NUMBER(10) NULL 
);

COMMENT ON COLUMN SHOP_ORDER_LOG.CCNT IS '변경 전, 신규구매 지정된 CCTV 대수';
COMMENT ON COLUMN SHOP_ORDER_LOG.BPRICE  IS '변경 전, 신규구매 기본단가';
COMMENT ON COLUMN SHOP_ORDER_LOG.PNO  IS '구독변경 신청번호 (FK) , 조회용';