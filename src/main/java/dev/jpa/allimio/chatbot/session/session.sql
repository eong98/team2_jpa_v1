...
CREATE TABLE CHAT_SESSION (
  ...
  CNO        NUMBER(10)                      NULL, -- 현재 위치한 옵션형 메뉴 (AI상담중인 경우 NULL) (FK -> CHAT_MENU.NO)
  CMODE      NUMBER(1)       DEFAULT 0   NOT NULL, -- 0: 옵션형 진행중, 1: AI상담 진행중, 2: 종료
  ENDFLOW    NUMBER(1)                       NULL, -- 현재 대기중인 세션 상태 (0: 만족도평가, 1: 불만족이유, 2: 불만족 이유 작성, 3:관리자 문의노출)
  ...
  CLOSEDAT   VARCHAR2(30)                    NULL, -- 실제 종료 시각 (CMODE=2 전환 시점)
  CREASON    NUMBER(1)                       NULL, -- 세션 종료 사유 (0: 사용자 종료, 1: 관리자연결, 2: 자동종료)
  ...
  STITLE     VARCHAR2(200)                   NULL, -- AI 상담내용 요약 타이틀
  READAT     VARCHAR2(30)                    NULL, -- 사용자가 마지막으로 이 채팅방을 읽은 시각
...
COMMENT ON COLUMN CHAT_SESSION.CMODE     IS '진행모드 - 0: 옵션형 진행중, 1: AI상담 진행중, 2: 종료';
...
COMMENT ON COLUMN CHAT_SESSION.CLOSEDAT IS '실제 종료 시각 (CMODE=2 전환 시점)';
COMMENT ON COLUMN CHAT_SESSION.CREASON  IS '세션 종료 사유 (0: 사용자종료, 1: 관리자연결, 2: 자동종료)';
...
COMMENT ON COLUMN CHAT_SESSION.STITLE    IS 'AI 상담내용 요약 타이틀';
COMMENT ON COLUMN CHAT_SESSION.READAT    IS '사용자가 마지막으로 이 채팅방을 읽은 시각';
COMMENT ON COLUMN CHAT_SESSION.ENDFLOW    IS '현재 대기중인 세션 상태 (0: 만족도평가, 1: 불만족이유, 2: 불만족 이유 작성, 3:관리자 문의노출)';
...
-- ALTER TABLE CHAT_SESSION ADD (
--   READAT VARCHAR2(30) NULL, -- 사용자가 마지막으로 이 채팅방을 읽은 시각
--   STITLE VARCHAR2(200) NULL -- AI 상담내용 요약 타이틀
-- );
-- ALTER TABLE CHAT_SESSION ADD (
--   ENDFLOW NUMBER(1) NULL
-- );


SELECT UDATE, READAT FROM CHAT_SESSION;

-- 테스트용 세션 번호 확인
SELECT NO, CMODE, STITLE, CDATE FROM CHAT_SESSION 
WHERE NO = '5692d24c-99b2-4d30-957c-2d799dac1a01'
ORDER BY CDATE DESC FETCH FIRST 5 ROWS ONLY;

SELECT content FROM CHAT_LOG 
WHERE SNO = '5692d24c-99b2-4d30-957c-2d799dac1a01'
ORDER BY CDATE DESC;



SELECT NO, CMODE, STITLE, CDATE FROM CHAT_SESSION ORDER BY CDATE DESC FETCH FIRST 5 ROWS ONLY;


SELECT * FROM CHAT_LOG WHERE SNO = 'c71e7af2-1e05-497f-bdfb-357da81ce567' ORDER BY NO;
SELECT ENDFLOW FROM CHAT_SESSION WHERE NO = 'c71e7af2-1e05-497f-bdfb-357da81ce567';